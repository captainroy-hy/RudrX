...
func NewPortForwardCommand(c types.Args, ioStreams velacmdutil.IOStreams) *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "port-forward SERVICE_NAME [options] [LOCAL_PORT:]REMOTE_PORT [...[LOCAL_PORT_N:]REMOTE_PORT_N]",
		Short: "Forward one or more local ports to a Pod of specified service",
		Long:  "Forward one or more local ports to a Pod of specified service",
